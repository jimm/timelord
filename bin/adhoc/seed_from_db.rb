...
Location.all.each do |loc|
  ...
  loc.codes.each do |code|
    ...
    code.work_entries.each do |w|
      puts "    WorkEntry.create(code: code, worked_at: '#{w.worked_at.to_s(:db)}', minutes: #{w.minutes || 0}, rate_cents: #{w.rate_cents || 'nil'}, fee_cents: #{w.fee_cents || 'nil'}, note: #{w.note.q || 'nil'})"
    end
  end
end
